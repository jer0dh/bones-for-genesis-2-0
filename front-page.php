...
add_filter( "genesis_attr_single-page-section",'bfg_genesis_attr_single_page_section'); // $attributes, $context );
...
add_action('genesis_after_content', 'bfg_genesis_get_sidebar');
add_action('genesis_after_content_sidebar_wrap', 'bfg_genesis_get_sidebar_alt');

function bfg_genesis_attr_single_page_section($attributes) {
	global $section, $bfg_page;
	$site_layout = bfg_get_site_layout($bfg_page);
...
$section = 0;
foreach ($bfg_pages as $page) {

	global $wp_query;
	$section += 1;
	$bfg_page = $page;
	$wp_query = new WP_Query(array('page_id' => $bfg_page, 'post_type' => 'page'));
...
/**
 * bfg_reset_sidebars() - was used to remove any hooks to the sidebars between pages
 * see bfg_site_sidebars() for details on why we changed this and no longer use it.
 *
 * Left for reference
 *
 */

/*function bfg_reset_sidebars () {
	remove_action( 'genesis_before_content_sidebar_wrap', 'bfg_genesis_get_sidebar_alt' );

	remove_action( 'genesis_after_content_sidebar_wrap', 'bfg_genesis_get_sidebar_alt' );

	remove_action( 'genesis_after_content', 'bfg_genesis_get_sidebar' );

	remove_action( 'genesis_before_content', 'bfg_genesis_get_sidebar' );
}*/



/**
 * bfg_site_layout - used to change the order of the sidebar HTML markup in an attempt to use bootstrap grid
 ...
 */
/*function bfg_site_layout() {
	if (func_num_args() > 0) {
		$site_layout = bfg_get_site_layout(func_get_arg(0));
	} else {
		$site_layout = bfg_get_site_layout();
	}
// Secondary Sidebar
	if ( in_array( $site_layout, array( 'sidebar-content-sidebar', 'sidebar-sidebar-content' ) ) ) {
		add_action( 'genesis_before_content_sidebar_wrap', 'bfg_genesis_get_sidebar_alt' );
	}
...
// Primary Sidebar
	...
	if ( in_array( $site_layout, array( 'sidebar-sidebar-content', 'sidebar-content' ) ) ) {
		add_action( 'genesis_before_content', 'bfg_genesis_get_sidebar' );
	}
}*/

/**
 * bfg_get_site_layout() - used to obtain the layout of a page because genesis_site_layout() would only return
...
function bfg_get_sidebar( $name = null ) {
	/**
...
/**
 * Replacing the genesis_get_sidebar() so to call the bfg_get_sidebar() instead of the WordPress get_slider()
 *
 */
function bfg_genesis_get_sidebar() {
...
/**
 * Replacing the genesis_get_sidebar_alt() so to call the bfg_get_sidebar() instead of the WordPress get_slider()
 *
 */
function bfg_genesis_get_sidebar_alt() {
